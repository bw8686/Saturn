...
class AudioPlayerHandler extends BaseAudioHandler
  ...
  Future<void> _init() async {
    ...
    _mediaItemSub =
        Rx.combineLatest3<int?, List<MediaItem>, bool, MediaItem?>(
    ...
    _player.playerEventStream.listen((event) {
      _broadcastState(event);
    }, onError: _playbackError);

    _player.shuffleModeEnabledStream.listen(
      (enabled) => _broadcastState(_player.playerEvent),
    );

    _player.loopModeStream.listen(
      (mode) => _broadcastState(_player.playerEvent),
    );
    ...
    _player.androidAudioSessionIdStream.listen((session) {
      if (!settings.enableEqualizer && Platform.isAndroid) return;

      equalizer.setEnabled(false);
    });

    // Throttle position updates to avoid spamming the main thread
    AudioService.position.throttleTime(const Duration(seconds: 1)).listen((
      position,
    ) {
  ...
  Future<void> playFromMediaId(
    ...
    if (index != -1) {
      ...
      Logger.root.severe(
        'playFromMediaId: MediaItem not found for mediaId: $mediaId',
      );
    }
  }
  ...
  Future<void> addQueueItem(MediaItem mediaItem) async {
    ...
    if (res != null) {
      await _player.addAudioSource(res);
  ...
  Future<void> insertQueueItem(int index, MediaItem mediaItem) async {
    ...
    final res = await _itemToSource(mediaItem);
    if (res != null) {
      await _player.insertAudioSource(index, res);
  ...
  Future<void> updateQueue(List<MediaItem> queue) async {
    ...
    if (queue.isNotEmpty) {
      await _player.setAudioSources(await _itemsToSources(queue));
      currentQueue = queue;
    } else {
  ...
  Future<void> removeQueueItem(MediaItem mediaItem) async {
    ...
    if (_player.shuffleModeEnabled) {
      ...
      await _player.removeAudioSourceAt(shuffledIndex);
      currentQueue.removeAt(shuffledIndex);
    } else {
  ...
  Future<void> moveQueueItem(int currentIndex, int newIndex) async {
    ...
    currentQueue.insert(newIndex, currentQueue[currentIndex]);
  ...
  Future<void> _startSession() async {
    ...
    _loadEmptyPlaylist().then(
      (_) => Logger.root.info('audio player initialized!'),
    );
  ...
  Future<void> _broadcastState(PlayerEvent event) async {
  ...
  Future _getTrackUrl(MediaItem mediaItem) async {
    ...
    List? streamPlaybackDetails = jsonDecode(
      mediaItem.extras?['playbackDetails'],
    );
    ...
    if (mediaItem.extras?['fallbackId'] != null) {
      ...
      streamPlaybackDetails = jsonDecode(
        mediaItem.extras?['playbackDetailsFallback'],
      );
    }
  ...
  Future _loadQueueAtIndex(
    ...
    await _player.setAudioSources(
      await _itemsToSources(newQueue),
      initialIndex: index,
  ...
  Future _onQueueEnd() async {
    ...
    switch (queueSource!.source) {
      ...
      case 'playlist':
        ...
        tracks = await deezerAPI.playlistTracksPage(
          queueSource!.id!,
          pos,
          nb: 25,
        );
    ...
    List<MediaItem> extraTracks = tracks
        .map<MediaItem>((t) => t.toMediaItem())
        .toList();
    addQueueItems(extraTracks);
  }

  void _playbackError(dynamic err, StackTrace stacktrace) {
    Logger.root.severe('Playback Error from audioservice: ${err.code}', err);
    if (err.code == 'abort' && err.message == 'Connection aborted') {
      return;
    }
  ...
  Future playShowEpisode(
    ...
    List<MediaItem> episodeQueue = episodes
        .map<MediaItem>((e) => e.toMediaItem(show))
        .toList();

    // Check if queue is the same by comparing MediaItem IDs
    bool isSameQueue =
        queueSource?.id != null &&
        showQueueSource.id == queueSource!.id &&
        episodeQueue.length == currentQueue.length &&
        episodeQueue.asMap().entries.every(
          (entry) => entry.value.id == currentQueue[entry.key].id,
        );
  ...
  Future playFromTrackList(
    ...
    //Generate media items
    List<MediaItem> trackQueue = tracks
        .map<MediaItem>((track) => track.toMediaItem())
        .toList();

    // Check if queue is the same by comparing MediaItem IDs
    bool isSameQueue =
        queueSource?.id != null &&
        trackQueueSource.id == queueSource!.id &&
        trackQueue.length == currentQueue.length &&
        trackQueue.asMap().entries.every(
          (entry) => entry.value.id == currentQueue[entry.key].id,
        );

    if (isSameQueue) {
      await skipToQueueItem(tracks.indexWhere((m) => m.id == trackId));
    } else {
      //Load and play
