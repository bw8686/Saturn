      ...
      children: <Widget>[
        Padding(
          ...
          child: SizedBox(
            ...
            child: const Stack(children: <Widget>[BigAlbumArt()]),
          ),
        ),
        //Right side
        SizedBox(
...
class _QualityInfoWidgetState extends State<QualityInfoWidget> {
  ...
  void _load() async {
    if (audioHandler.mediaItem.value == null) return;
    final data = StreamServerDart.instance.getStreamInfo(
...
class LyricsIconButton extends StatelessWidget {
  ...
  const LyricsIconButton(this.width, {super.key, this.afterOnPressed});

  @override
  Widget build(BuildContext context) {
    Track track = Track.fromMediaItem(
      GetIt.I<AudioPlayerHandler>().mediaItem.value!,
    );
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (cache.checkTrackFavorite(
                ...
                //Remove from library
                setState(
                  () => cache.libraryTracks?.remove(
                    audioHandler.mediaItem.value!.id,
                  ),
                );
                await deezerAPI.removeFavorite(
                  audioHandler.mediaItem.value!.id,
                );
                await cache.save();
              } else {
...
class _BigAlbumArtState extends State<BigAlbumArt> with WidgetsBindingObserver {
  ...
  void initState() {
    super.initState();
    _pageController = PageController(initialPage: audioHandler.currentIndex);
  ...
  void _handleMediaItemChange(MediaItem? item) async {
    ...
    final targetPage = audioHandler.queue.value.indexWhere(
      ...
      children: <Widget>[
        ...
        IconButton(
          ...
          onPressed: () async {
            ...
            final Offset buttonOffset = buttonRenderBox.localToGlobal(
              buttonRenderBox.size.center(Offset.zero),
            );
            //Navigate
            await Navigator.of(context).push(
...
class _SeekBarState extends State<SeekBar> {
  ...
  double get position {
    ...
    double p = audioHandler.playbackState.value.position.inMilliseconds
        .toDouble();
            ...
            SizedBox(
              ...
              child: Slider(
                focusNode: FocusNode(
                  ...
                  skipTraversal: true,
                ), // Don't focus on Slider - it doesn't work (and not needed)
                value: position,
                max: duration,
...
class _QueueScreenState extends State<QueueScreen> with WidgetsBindingObserver {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body:
          shuffleModeEnabled // No manual re-ordring in shuffle mode
          ? ListView.builder(
...
class _QueueScreenNoDRGState extends State<QueueScreenNoDRG> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: FreezerAppBar('Queue'.i18n),
